    ...
    def self.add_url_to_multi(multi, url, url_queue, responses, options)
      easy = Curl::Easy.new(url) do |curl|
        ...
        curl.on_success do |c|
          ...
          if klass
            begin
              feed = klass.parse(xml, Proc.new{|message| raise "Error while parsing [#{url}] #{message}" })
      ...
      easy = Curl::Easy.new(feed.feed_url) do |curl|
        ...
        curl.on_success do |c|
          begin
            ...
            updated_feed = Feed.parse(c.body_str){ |message| raise "Error while parsing [#{feed.feed_url}] #{message}" }
